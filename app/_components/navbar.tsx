        ...
        <div className="relative items-center py-3 px-4 mx-auto container sm:px-8 md:flex md:space-x-6">
                ...
                <ul className={`flex-1 justify-between items-center pt-8 md:pt-0 md:text-sm md:font-medium md:flex md:mt-0 ${state ? 'absolute inset-x-0 px-4 border-b bg-white md:border-none md:static' : 'hidden'}`}>
                    <div className="items-center space-y-5 md:flex md:space-x-6 md:space-y-0 md:ml-12">
                        {
                            navigation.map((item, idx) => (
                                <li className={`text-gray-500 ${isScrolled ? "hover:text-slate-800": "hover:text-slate-300"} font-semibold`} key={idx}>
                    ...
                    <li className="order-2 py-5 md:py-0">
                        <a href="/contact" className={`py-2 px-5 rounded-lg font-medium text-center ${(isScrolled) ? "text-white bg-slate-800 hover:bg-slate-700 active:bg-slate-900": variant === "light"? "text-white bg-slate-800 hover:bg-slate-700 active:bg-slate-900":"text-slate-800 bg-slate-200 hover:bg-slate-100 active:bg-slate-300"} duration-150 block md:py-3 md:inline`}>
                            Nous contacter
                        </a>
                    </li>
                </ul>
